class OrderTypeFileValidator < ActiveModel::Validator
  ...
  def validate(record)
    ...
    validate_order_type_structure(hash)
  ...
  def validate_field_definitions(hash)
    return unless hash[:order_type]

    if should_have_key?(:fields, in: hash[:order_type], as: Hash)
      validate_field_def_set(hash[:order_type][:fields])
    end
  end

  def validate_field_def_set(fields)
    fds = CustomFields::OrderTypeFieldDefSet.new(fields)
