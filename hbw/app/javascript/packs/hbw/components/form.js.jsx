...
(React, jQuery, Error, DateTime, Group, Select, Submit, SubmitSelect, User, Pending,
  String, Text, Checkbox, Static, CallbacksMixin, SelectTable, FileList, FileUpload) => React.createClass({
  ...
  iterateControls (fields) {
    return [...fields].map(field => <div key={field.name} className="row">{this.formControl(field.name, field)}</div>);
  },
