...
describe API::V1::UsersController, type: :request do
  resource I18n.t('doc.users.resource') do
    ...
    put '/api/users/:email' do
      ...
      parameter :password_salt,      I18n.t('doc.users.parameters.password_salt'),      scope: :user
      parameter :encrypted_password, I18n.t('doc.users.parameters.encrypted_password'), scope: :user

      let(:middle_name)        { 'Jay' }
      let(:email)              { 'demo@example.com' }
      let(:blocked)            { true }
      let(:external)           { true }
      let(:password_salt)      { 'U0N9NNPINLPAKK9ABVBO9CALOA4OSZXB' }
      let(:encrypted_password) { '3f338baf4c2b847d8e81fda3f4bd982aecb3f568' }
    ...
    post '/api/users' do
      ...
      parameter :password_salt, I18n.t('doc.users.parameters.password_salt'),      required: true, scope: :user
      parameter :password_salt, I18n.t('doc.users.parameters.encrypted_password'), required: true, scope: :user
